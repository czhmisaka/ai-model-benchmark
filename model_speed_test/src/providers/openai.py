...
class OpenAIProvider(BaseLLMProvider):
    ...
    async def chat(
        ...
        **kwargs
        ...
        try:
            ...
            return LLMResponse(error=f"Connection error: {str(e)}")
        except Exception as e:
            return LLMResponse(error=f"Request error: {str(e)}")
    ...
    def _parse_response(self, data: Dict[str, Any]) -> LLMResponse:
        ...
        try:
            ...
            return LLMResponse(error=f"Parse error: {str(e)}", raw_response=data)
        ...
        try:
            async with session.post(
                ...
                async for line in response.content:
                    ...
                    chunk = await self._parse_stream_chunk(data_str, first_chunk, in_think)
                    if chunk:
                        # 更新 think 状态
                        if chunk.is_think:
                            in_think = True
    ...
    async def _parse_stream_chunk(
        ...
        try:
            ...
            if "choices" in data:
                ...
                content = delta.get("content") or ""  # answer 内容
                ...
                is_first = first_chunk and not content
                is_think = False
                is_think_end = False
                
                # 检测 think 标签（某些模型使用标签）
                if not in_think and not is_think_end:
                    if '<think>' in content:
                        is_think = True
                    if '</think>' in content:
                        is_think_end = True
                ...
                return StreamChunk(
                    ...
                    is_think=bool(reasoning_content),  # 有 reasoning_content 就是 think
